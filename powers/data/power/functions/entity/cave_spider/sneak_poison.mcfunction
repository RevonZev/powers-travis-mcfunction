...
execute at @s[scores={power.cooldown1=1}] run playsound minecraft:entity.spider.ambient hostile @a[distance=..5] ~ ~ ~ .5 .8 1
## If cave spider player sneaked for more than one tick; run green particles
execute at @s[scores={power.cooldown1=1..}] run particle minecraft:dust 0 0.6 0 1 ~ ~ ~ 5.5 0 5.5 0.1 10 normal
## If cave spider player sneaked for sixty tick; give poison to all player all around
execute at @s[scores={power.cooldown1=60}] run effect give @e[distance=0.1..5] minecraft:poison 3 1 true
...
execute as @s unless score @s power.sneak matches 1.. run scoreboard players set @s power.cooldown1 0